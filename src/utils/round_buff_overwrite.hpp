//
// Created by Serg on 009 09.01.21.
//
...
// При переполнении затираются старые значения.
// Не параллельно безапасен.
template<typename T, size_t MaxSize>
class RoundBuffOverwrite {
 public:
  RoundBuffOverwrite() : ind_read_(0), ind_write_(0) {
  }
  void Reset() {
    ind_read_ = 0;
    ind_write_ = 0;
  ...
  bool PushBack(const T &value) {
    ...
    if(next_write == ind_read_) { // полный
      ind_read_ = (ind_read_ == MaxSize-1) ? 0 : ind_read_ + 1;
    }
  ...
  // Возвращает реально прочитанное кол-во элементов.
  size_t ReadCopy(T *buf, size_t len) const {
    if(ind_read_ == ind_write_)
      return 0;
    size_t current_size = Size();
    if(len > current_size) {
  ...
  // Сдвигаем указатель чтения на заданное число позиций (удаляем данные).
  void Delete(size_t len) {
    if(ind_read_ == ind_write_)
      return;
    size_t current_size = Size();
    if(len > current_size) {

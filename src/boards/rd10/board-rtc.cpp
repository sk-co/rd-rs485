...
void RtcInit(uint32_t init_time) {
...
//    HAL_RTC_SetTime(&RtcHandle, &time, RTC_FORMAT_BIN);
    ...
    if(init_time) {
      ...
      SysTime_t sys_time = {init_time, 0};
      SysTimeSet(sys_time);
    }
  }
}
...
uint32_t RtcSetTimerContext(void) {
  RtcTimerContext.Time =
      (uint32_t) RtcGetCalendarValue(&RtcTimerContext.CalendarDate, &RtcTimerContext.CalendarTime);
...
void RtcSetAlarm(uint32_t timeout) {
  ...
  if ((int64_t) (MIN_ALARM_DELAY + McuWakeUpTimeCal)
      < (int64_t) (timeout - RtcGetTimerElapsedTime())) {
    LpmSetStopMode(board::LPM_RTC_ID, board::LPM_ENABLE);
  } else {
    LpmSetStopMode(board::LPM_RTC_ID, board::LPM_DISABLE);
  }
  // In case stop mode is required
  if (board::LpmGetMode() == board::LPM_STOP_MODE) {
...
uint32_t RtcGetTimerValue(void) {
  RTC_TimeTypeDef time;
  RTC_DateTypeDef date;
  auto calendarValue = (uint32_t) RtcGetCalendarValue(&date, &time);
...
static uint64_t RtcGetCalendarValue(RTC_DateTypeDef *date, RTC_TimeTypeDef *time) {
  ...
  correction = ((date->Year % 4) == 0) ? DAYS_IN_MONTH_CORRECTION_LEAP
      : DAYS_IN_MONTH_CORRECTION_NORM;
  seconds += (DIVC((date->Month - 1) * (30 + 31), 2) -
...
uint32_t RtcGetCalendarTime(uint16_t *milliseconds) {
  ...
  auto seconds = (uint32_t) (calendarValue >> N_PREDIV_S);
